...
const maxCount = computed(() =>
  Math.max(1, ...biz.chartData.map((d) => d.count)),
);
  ...
  <div class="space-y-6">
    <div>
      <h2 class="text-2xl font-black tracking-tight">
        {{ ui.t("dashboard.greeting")
        }}<span v-if="auth.displayName">, {{ auth.displayName }}</span>
      </h2>
      <p class="mt-1 text-sm opacity-60">{{ ui.t("dashboard.subtitle") }}</p>
    </div>

    <RouterLink
      v-if="biz.subscriptionWarning"
      to="/to-lov"
      ...
      <span class="flex-1">
        <span class="block text-sm font-bold">
          {{
            biz.subscriptionExpired
              ? "Obunangiz tugagan, to'lov qiling"
              : `Obuna ${biz.subscriptionDaysLeft} kun ichida tugaydi`
          }}
        </span>
        <span class="mt-1 block text-sm leading-relaxed opacity-65"
          >To'lov chekini yuborib, xizmatdan foydalanishni davom ettiring.</span
        >
      </span>
      <AppIcon
        name="chevronRight"
      ...
      <section class="ftw-card p-5">
        ...
        <div class="mt-6 flex items-end gap-1.5">
          <div
            ...
            <span
              class="text-[10px] font-bold text-primary opacity-0 transition-opacity group-hover:opacity-100">
              {{ day.count }}
            </span>
            <div
            ...
            <span class="text-[10px] font-medium opacity-45">{{
              day.label
            }}</span>
          </div>
        </div>
